...
### Fixed
- Possible deadlock when GPU indices are provided
- Remove out-of-date `./tests` scripts (tests are inside the package now)

### Changed
- Use faster (and GIL-friendly) loudness computation 
- Use faster resampling
- Use threadpool instead of processpool for resampling/mixing to avoid IPC overhead

### Added
- Benchmark script for embedding pipeline
